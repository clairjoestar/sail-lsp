...
fn compute_semantic_diagnostics_impl(
    ...
    if let Some(ast) = file.core_ast() {
        ...
        // Pattern completeness is now handled by the type-aware checker in
        // `typecheck::Checker::check_match_exhaustiveness`. The old
        // heuristic guessed the scrutinee type by name; the typechecker
        // knows it precisely, so we no longer need this pass.
        let _ = (extra_enum_members, extra_union_variants);
